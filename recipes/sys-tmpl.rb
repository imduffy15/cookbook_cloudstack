...
node["cloudstack"]["systemvms"].each do |systemvm|
  ...
  execute "./cloud-install-sys-tmplt -m #{node["cloudstack"]["storage"]["secondary"]} -f #{filename} -h #{systemvm["hypervisor"]} -t #{node["cloudstack"]["hypervisors"][systemvm["hypervisor"]]}" do
